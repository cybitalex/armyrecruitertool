  ...
  async getRecruiterStatsAggregated(recruiterId: string): Promise<{
    ...
    try {
      ...
      const [
        ...
        // QR scan tracking — parallel with the rest. Conversions are only counted
        // when the linked recruit/survey still exists, so deleted applications and
        // surveys don't inflate the stats.
        db
        .select({
